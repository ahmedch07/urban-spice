...
type AddToCartDialogProps = {
  product: Product;
  extraTopping?: { name: string; price: number };
  onConfirm: (includeExtraTopping: boolean) => void;
  onCancel: () => void;
};

export function AddToCartDialog({ product, extraTopping, onConfirm, onCancel }: AddToCartDialogProps) {
  const [includeExtraTopping, setIncludeExtraTopping] = useState(false);
    ...
    <div className="fixed inset-0 z-50 grid place-items-center bg-[#32170e]/50 p-4 backdrop-blur-sm">
      <section role="dialog" aria-modal="true" aria-labelledby="add-to-cart-dialog-title" className="w-full max-w-sm rounded-3xl border border-orange-100 bg-[#fffaf3] p-6 text-center shadow-2xl">
        ...
        {extraTopping && (
          <button
            type="button"
            onClick={() => setIncludeExtraTopping((selected) => !selected)}
            className={`mt-4 flex w-full items-center justify-between rounded-2xl border px-4 py-3 text-left text-sm font-bold transition ${includeExtraTopping ? "border-orange-400 bg-orange-50 text-[#4a0a18]" : "border-orange-100 bg-white text-[#6b3422]"}`}
          >
            <span>{extraTopping.name}</span>
            <span>+Rs. {Math.round(extraTopping.price).toLocaleString()}</span>
          </button>
        )}
        <div className="mt-6 grid grid-cols-2 gap-3">
          ...
          <Button type="button" size="lg" className="bg-[#32170e] text-white hover:bg-[#572314]" onClick={() => onConfirm(includeExtraTopping)}>Yes, add it</Button>
        </div>
      </section>
    </div>
  );
