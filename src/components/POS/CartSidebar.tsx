...
export default function CartSidebar({
  ...
  const cartContent = (
    <div className="w-full h-full min-h-0 bg-slate-900 flex flex-col select-none">
      {/* 1. Header & Order Type Selector */}
      <div className="shrink-0 p-2.5 border-b border-slate-800 space-y-2 bg-slate-900/90">
        <div className="flex items-center justify-between gap-2">
          <div className="min-w-0 flex items-center space-x-2">
            <ShoppingBag className="w-5 h-5 text-amber-400" />
            <h2 className="truncate font-bold text-base text-slate-100">
              {activeOrderId ? 'Active Table Tab' : 'Current Order'}
            </h2>
            {cart.length > 0 && (
              <span className="shrink-0 px-2 py-0.5 rounded-full text-[10px] font-mono bg-amber-500/20 text-amber-400 font-bold border border-amber-500/30">
        ...
        <div className="grid grid-cols-3 gap-1 p-1 bg-slate-950 rounded-xl border border-slate-800">
          <button
            ...
            className={`py-1.5 px-1 rounded-lg text-xs font-bold flex items-center justify-center space-x-1 transition-all ${
              orderType === 'DINE_IN'
        ...
        {orderType === 'DINE_IN' && (
          <div className="bg-slate-950 border border-slate-800/80 rounded-xl p-2 space-y-1.5">
        ...
        {orderType === 'DELIVERY' && (
          <div className="bg-slate-950 border border-slate-800/80 rounded-xl p-2 space-y-2">
            <div className="flex items-center justify-between">
              <span className="text-[11px] font-bold text-amber-400 uppercase tracking-wider flex items-center space-x-1">
      ...
      {/* 2. Cart Items List */}
      <div className="flex-1 min-h-0 overflow-y-auto overscroll-contain p-3 space-y-2.5" aria-label="Order items">
        {cart.length === 0 ? (
          ...
          cart.map((item) => (
            <div
              key={item.cartId}
              className="p-3 bg-slate-950 border border-slate-700 rounded-2xl space-y-2 shadow-sm hover:border-amber-500/50 transition-colors"
            >
              {/* Product Title & Remove */}
              <div className="flex items-start justify-between">
                <div className="min-w-0 pr-2">
                  <h4 className="text-sm font-bold text-slate-100 tracking-tight leading-tight break-words">
                    {item.productName}
                  </h4>
                  {item.isPizza && (
                ...
                <button
                  ...
                  className="shrink-0 p-1.5 text-slate-400 hover:text-red-400 transition-colors"
                  title="Remove Item"
                >
                  <Trash2 className="w-3.5 h-3.5" />
                </button>
              ...
              <div className="flex items-center justify-between pt-1 border-t border-slate-900">
                <div className="flex items-center space-x-1.5">
                  <button
                    ...
                    className="w-7 h-7 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-100 flex items-center justify-center font-bold text-xs transition-colors"
                  >
                    <Minus className="w-3 h-3" />
                  </button>
                  <span className="w-7 text-center font-bold text-sm font-mono text-slate-100">
                    {item.quantity}
                  </span>
                  <button
                    onClick={() => onUpdateQuantity(item.cartId, 1)}
      ...
      <div className="shrink-0 p-2.5 border-t border-slate-800 bg-slate-900/95 space-y-1.5">
        {/* Subtotal & Discounts */}
        <div className="max-h-[8vh] overflow-y-auto overscroll-contain pr-1 space-y-1 text-xs">
  ...
  return (
    <>
      ...
      <aside className="hidden lg:flex w-80 xl:w-96 min-h-0 border-l border-slate-800 h-full shrink-0 z-20 shadow-2xl">
        {cartContent}
      </aside>
